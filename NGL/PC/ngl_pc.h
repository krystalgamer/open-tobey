...
struct nglTexture
{
	...
	void *DXSurface;  // D3D surface (aka texture of level 0).
...
enum
{
    NGLPRIM_PNTLIST   ,      // Collection of isolated points.
    NGLPRIM_LINELIST  ,       // List of isolated straight line segments. Count must be >= 2 and even !
    NGLPRIM_LINESTRIP ,      // Single polyline. Count must be >= 2 !

    NGLPRIM_TRILIST   ,   // Sequence of isolated triangles. Each group of three vertices defines a separate triangle. 

    NGLPRIM_TRISTRIP  ,  // Triangle strip (first one must be clock-wise).
	NGLPRIM_TRIFAN    ,    // Triangle fan.
    NGLPRIM_QUADLIST ,       // Sequence of isolated quads (4 vertices / quad !).
};
...
struct nglVertexBuffer
{
	...
	void* IndexBuffer;	// Unused. To be removed.
...
struct nglMeshSection
{
	...
	void *PB;   // Pushbuffer for the section (0 if no PB rendering).
