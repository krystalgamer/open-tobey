...
public:
  ...
  public:
	...
	#ifdef TARGET_PS2
  typedef std::vector<light_rec,malloc_alloc> light_list_t;
	#else
  typedef std::vector<light_rec> light_list_t;
	#endif
  light_list_t lights;
...
public:
  ...
  std::vector<light_properties> point_light_props;
  std::vector<dir_light> dir_lights;
  std::vector<light_properties> dir_light_props;
  color ambient_factor;
  typedef std::pair<float,int> light_dist;
  std::vector<light_dist> point_light_dists;
