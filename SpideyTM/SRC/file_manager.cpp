...
printf("Attempting to open file %s\n", image_filename.c_str());
  ...
  if (paths == NULL)
    return false;
  std::vector<stringx> &path_list = paths->get_paths();
  std::vector<stringx>::iterator it = path_list.begin();
  std::vector<stringx>::iterator it_end = path_list.end();
