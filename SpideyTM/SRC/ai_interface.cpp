...
std::list<ai_interface *> ai_interface::all_ai_interfaces;
...
{
	std::list<ai_interface *>::iterator ai = all_ai_interfaces.begin();
...
//BIGCULL
  ...
  */
  ...
  std::vector<ai_goal *>::iterator g = goals.begin();
  while(g != goals.end())
  {
    delete (*g);
    ++g;
...
void ai_interface::copy(ai_interface *b)
{
  ...
  std::vector<ai_goal *>::iterator g = b->goals.begin();
  while(g != b->goals.end())
  {
...
{
	std::list<ai_interface *>::iterator ai = all_ai_interfaces.begin();
  	std::list<ai_interface *>::iterator ai_end = all_ai_interfaces.end();
  while(ai != ai_end)
...
{
	std::vector<ai_interface *>::iterator ait = attackers.begin();
...
void ai_interface::frame_advance(time_value_t t)
{
  ...
  std::vector<ai_goal *>::iterator g = goals.begin();
  std::vector<ai_goal *>::iterator g_end = goals.end();
  while(g != g_end)
  {
...
{
	std::vector<ai_goal *>::const_iterator g = goals.begin();
  	std::vector<ai_goal *>::const_iterator g_end = goals.end();
  while(g != g_end)
  {
    if((*g)->type == goal_name)
      return(*g);
