...
std::vector<path_graph_edge*>* patrol_edges_workspace;


void path_graph_system_construct()
{

  patrol_edges_workspace = NEW std::vector<path_graph_edge*>;
}
...
static void sort_edges_by_weight(std::vector<path_graph_edge *> &edges)
{
  if(edges.size() > 1)
    qsort( &(*edges.begin()), edges.size(), sizeof(path_graph_edge *), compare_path_graph_edge_ptr_by_weight );
}

static void sort_edges_by_bias(std::vector<path_graph_edge *> &edges)
{
  if(edges.size() > 1)
...
{
	std::vector<path_graph_edge *>::iterator i = edges.begin();
  while(i != edges.end())
  {
    if(*i != NULL && *i == edge)
...
void path_graph_node::copy(const path_graph_node &b)
{
  ...
  std::vector<path_graph_edge *>::const_iterator i = b.edges.begin();
  while(i != b.edges.end())
  {
    edges.push_back(*i);
    ++i;
...
void path::copy(const path *b)
{
  ...
  std::vector<path_graph_node *>::const_iterator i = b->nodes.begin();
  while(i != b->nodes.end())
  {
    nodes.push_back((*i));
...
const path_graph_edge *path::get_edge(const path_graph_node *n1, const path_graph_node *n2) const
{
  ...
  std::vector<path_graph_edge *>::const_iterator i = n1->edges.begin();
...
void path::render( camera* camera_link, color32 col, rational_t thickness)
{
	...
	std::vector<path_graph_node *>::iterator i = nodes.begin();
  while(i != nodes.end())
  {
...
void path_graph::render( camera* camera_link, color32 col, rational_t thickness, char level)
{
  ...
  std::vector<path_graph_edge *>::iterator j = edges.begin();
  while(j != edges.end())
...
bool path_graph::node_in_graph(const path_graph_node *node) const
{
	...
	std::vector<path_graph_node *>::const_iterator i = nodes.begin();
  while(i != nodes.end())
  {
    if(*i != NULL && (*i) == node)
      return(true);
...
{
	std::vector<path_graph_edge *>::const_iterator j = edges.begin();
  while(j != edges.end())
  {
...
path_graph_node *path_graph::get_nearest_node(const vector3d &pt, bool smart) const
{
  ...
  if(!nodes.empty())
  {
    if(smart)
    {
      static std::vector<path_graph_edge *> edge_list;
      edge_list.reserve(edges.size());
      edge_list.resize(0);

      vector3d hit, hit_norm;

	  std::vector<path_graph_edge *>::const_iterator i = edges.begin();
	  std::vector<path_graph_edge *>::const_iterator i_end = edges.end();
      while(i != i_end)
      {
...

    {
		...
		std::vector<path_graph_node *>::const_iterator i_end = nodes.end();
...
//    warning("%s: Node to edge ratio exceeded (nodes: %d, edges: %d, max edges: %d)!", id.c_str(), nodes.size(), edges.size(), (int)((((rational_t)nodes.size())*_EDGE_TO_NODE_RATIO) + 0.5f));
  ...
  std::vector<path_graph_edge *> bad_patrol_edges;
  ...
  if(!bad_patrol_edges.empty())
  {
    ...
    int counter = 0;
	std::vector<path_graph_edge *>::const_iterator i = bad_patrol_edges.begin();
  ...
  if(edge != NULL && edge->is_patrol())
  {
    ...
    for(int i=0; i<2; ++i)
    {
      ...
      if(!node->visited())
      {
		...
		std::vector<path_graph_edge *>::iterator e = node->edges.begin();
...

{
  ...
  std::vector<path_graph_node *>::const_iterator j = nodes.begin();
  while(j != nodes.end())
  {
    if(*j != NULL)
    {
		std::vector<path_graph_edge *>::const_iterator i2 = (*j)->edges.begin();
...
void path_graph::clear()
{
  ...
  std::vector<path_graph_node *>::iterator j = nodes.begin();
  while(j != nodes.end())
  {
    if(*j != NULL)
    {
  ...
  else
  {
    if(dest != NULL)
    {
      if(!cur->is_bias_sorted())
      {
		...
		std::vector<path_graph_edge *>::iterator i = cur->edges.begin();
		std::vector<path_graph_edge *>::iterator i_end = cur->edges.end();
        while(i != i_end)
        {
          path_graph_edge *edge = (*i);
          ++i;
...
bool path_graph::find_all_paths_recurse(path_graph_node *cur, path &cur_path, std::vector<path> &best_paths, int &depth, int &call_count, const unsigned short flags, rational_t max_dist, bool retval)

{
  ...
  if(cur->is_flagged(flags))
  {
	...
	std::vector<path>::iterator i = best_paths.begin();
...
int path_graph::find_all_paths(const vector3d &start, std::vector<path> &found_paths, const unsigned short flags, bool sort)
{
...
int path_graph::find_all_paths(path_graph_node *start, std::vector<path> &found_paths, const unsigned short flags, bool sort)
...
bool path_graph::get_next_patrol_point(path_graph_node *last, path_graph_node *curr, vector3d &next_point, int patrol_id)
{
...

  {
	...
	std::vector<path_graph_edge *>::iterator i = curr->edges.begin();
...
void path_graph::use_path(path *path, rational_t additional_weight)
{
  ...
  std::vector<path_graph_node *>::iterator i = path->nodes.begin();
