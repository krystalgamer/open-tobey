...
void sound_group::copy(const sound_group &b)
{
  ...
  std::vector<sg_entry>::const_iterator sgei = b.entries.begin();
...
{
	VALIDATE_SIZE(sound_group, 0x40);

	VALIDATE(sound_group, name, 0x0);
	VALIDATE(sound_group, maxhistory, 0x20);
...
void validate_sg_entry(void)
{
	...
	VALIDATE(sg_entry, name, 0x0);

	VALIDATE(sg_entry, pitch, 0x24);
	VALIDATE(sg_entry, pitch_var, 0x28);

	VALIDATE(sg_entry, volume, 0x2C);
	VALIDATE(sg_entry, volume_var, 0x30);

	VALIDATE(sg_entry, min_dist, 0x34);
	VALIDATE(sg_entry, max_dist, 0x38);

	VALIDATE(sg_entry, field_3C, 0x3C);

	VALIDATE(sg_entry, is_voice, 0x40);

	VALIDATE(sg_entry, last_id_played, 0x44);
	VALIDATE(sg_entry, field_48, 0x48);
}
...
{
	PATCH_PUSH_RET(0x0060BD60, sg_entry::copy);
}
