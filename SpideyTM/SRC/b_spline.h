...
protected:
  ...
  std::vector<vector3d> control_pts;
  std::vector<vector3d> curve_pts;
  int sub_divisions;
...
public:
  ...
  inline void build(const std::vector<vector3d> &pts, int sub_divisions, eSplineType type = _B_SPLINE)
  {
    clear();
  ...
  inline const std::vector<vector3d> &get_control_pts() const  { return(control_pts); }
  inline const std::vector<vector3d> &get_curve_pts() const    { return(curve_pts); }
