...
class text_font
{
	friend void validate_text_font(void);
	friend void patch_text_font(void);
  class glyph_info
  {
...
class typeface_def
{
  ...
  struct inter_kern
  {
    ...
    {
      letter_pair = std::pair<int,int>(l1,l2);
      kern = k;

    }
	std::pair<int,int> letter_pair;
    int kern;
  };


  std::vector<inter_kern> interletter_kern_info;
