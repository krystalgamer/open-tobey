...
void beam::purge_effects()
{
  if ( !effects.empty() )
  {
	  std::vector<beam_effect*>::iterator i = effects.begin();
    for ( ; i!=effects.end(); ++i )
    {
      beam_effect *eff = *i;
...
void beam::copy_instance_data( const beam& b )
{
  ...
  if ( !b.effects.empty() )
  {
	  std::vector<beam_effect*>::const_iterator i = b.effects.begin();
...
{
	std::vector<beam_effect*>::iterator i = effects.begin();
	std::vector<beam_effect*>::const_iterator i_end = effects.end();
...
//    entity::set_flag( EFLAG_MISC_COMP_SECT_THIS_FRAME, true );
    if ( sec )
    {
      ...
      // check all regions recursively
		std::vector<region_node*> new_regions;
	  ...
	  std::vector<region_node*>::const_iterator k = new_regions.begin();
	  std::vector<region_node*>::const_iterator k_end = new_regions.end();
      for ( ; k!=k_end; ++k )
        add_region( *k );
    }
    else
...
void beam::_intersect( region_node* rn, std::vector<region_node*>& new_regions ) const
{
#ifdef BEAMCULL
