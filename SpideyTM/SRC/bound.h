...
public:
  ...
  EXPORT bounding_box(const vector3d& _min,const vector3d& _max) 
    : vmin(_min), vmax(_max)
  {}

  EXPORT bounding_box(const vector3d& _ctr,rational_t radius)  // from sphere
    : vmin(_ctr), vmax(_ctr)
  {
  ...
  EXPORT vector3d size() const
  {
    return vmax-vmin;
  }

  EXPORT vector3d center() const
  {
    return (vmax+vmin)*0.5F;
  }

  EXPORT float radius() const // radius needed to include box corners (measured from center)
  {
    return (vmax-vmin).length()*0.5F;

  }

  EXPORT float xz_radius() const
  {
  ...
  EXPORT void accumulate(const vector3d& p) 
  {
  ...
  EXPORT bool intersect(const bounding_box& b) const
  {
  ...
  // point within box?
  EXPORT bool intersect(const vector3d& p) const
  {
    return ( p.x >= vmin.x &&
  ...
  EXPORT bool xz_intersect( const vector3d& p ) const
  {
  ...
  EXPORT bool intersect( const vector3d& p, rational_t r ) const
  {
    return ( p.x+r > vmin.x &&
             p.y+r > vmin.y &&
