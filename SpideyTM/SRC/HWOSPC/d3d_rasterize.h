...
class vert_lit_buf : public ref
{
  vert_lit_buf( const vert_buf& vb );
public:
  vert_lit_buf(); // to construct a global vertex workspace type buffer
  vert_lit_buf(int _max_size); // to construct a buffer of a particular size
	~vert_lit_buf();
  ...
  hw_rasta_vert_lit * begin() const { assert(locked); return verts; }
  hw_rasta_vert_lit * end()   const { assert(locked); return verts + max_size; }


private:
  hw_rasta_vert_lit * verts;
  int max_size;
  bool locked;
  bool optimized;
  friend class hw_rasta;
...
public:
  ...
  void send_indexed_vertex_list( const vert_lit_buf& buf, int num_verts, 
                                 const unsigned short* indices, int num_indices,
                                 unsigned flags=SEND_VERT_NORMAL );
